...
namespace WazFactor\EventManager;
...
class EventManager
{
	...
	public function setSubscriber( SubscriberInterface $subscriber ): void
	{
		...
		foreach ( $subscriber->getSubscribedEvents() as $tag => $parameters ) {
			$this->addSubscriberCallback( $subscriber, $tag, $parameters );
		}
	}
	
	public function unsetSubscriber( RemoveSubscriberInterface $subscriber ): void
	{
		foreach ( $subscriber->getUnSubscribedEvents() as $tag => $functions ) {
			if (is_string( $functions ) ) {
				$this->removeSubscriberCallback( $subscriber, $tag, $functions );
			} elseif ( is_array( $functions ) ) {
				foreach( $functions as $parameters ) {
					$this->removeSubscriberCallback( $subscriber, $tag, $parameters );
				}
			}
		}
	}
	 ...
	 */
	public function add( string $tag, callable $callback, int $priority = 10, int $args = 1 ): void
	{
	 ...
	 * @param bool|mixed  $callback    The callback function to check with WordPress Plugin API.
	 *
	 * @return bool|int
	 * @uses has_filter()
	 *
	 */
	public function has( string $tag, mixed $callback = false ): bool|int
	{
	 ...
	 * @param mixed|null  $args    An array typically, args supplied to functions hooked to $hook_name.
	 *
	 *@uses do_action_ref_array()
	 *
	 */
	public function execute( string $tag, mixed $args = null ): void
	{
	 ...
	 * @return void
	 */
	private function addSubscriberCallback( SubscriberInterface $subscriber, string $tag, $parameters ): void
	{
	 ...
	 */
	private function removeSubscriberCallback( RemoveSubscriberInterface $subscriber, string $tag, $params ): void
	{
		if ( is_string( $params ) ) {
